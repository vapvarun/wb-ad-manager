# WB Ad Manager - One-Pager Sales Sheet

**Purpose:** Quick reference for sales conversations and partnerships
**Format:** Single-page layout (can be designed as PDF)
**Audience:** Potential customers, partners, reviewers

---

## Header Section

### Logo
[WB Ad Manager Logo]

### Hero Image
![WB Ad Manager Pro — Advertiser Portal overview](../../docs/website/images/pro/portal-overview.png)

### Tagline
**Complete Ad Management for WordPress**

### One-Line Description
The all-in-one plugin for display ads, affiliate links, and community monetization.

---

## The Problem

WordPress site owners struggle with:
- Multiple plugins for different monetization needs
- No unified tracking across ad types
- Complex setup for simple tasks
- Poor integration with community platforms

---

## The Solution

**WB Ad Manager** consolidates all monetization into one plugin:

| Capability | What It Does |
|------------|--------------|
| Display Ads | Image, HTML, rich content, AdSense |
| Affiliate Links | Cloaking, tracking, broken link detection |
| Placements | 14+ options: content, popup, sticky, widget |
| Targeting | Device, user role, schedule, content type |
| Community Ads | Native BuddyPress & bbPress integration |
| Analytics | Impressions, clicks, CTR tracking |

---

## Free vs Pro

### Free (WordPress.org)
- Unlimited ads & links
- All ad types & placements
- Full targeting options
- Affiliate link management
- BuddyPress/bbPress ads
- Basic analytics

### Pro (Premium Add-on)
Everything in Free, plus:
- Classified marketplace
- Advertiser self-service portal
- Wallet system — credits via WooCommerce, Paid Memberships Pro, or MemberPress adapters (bundled Wbcom Credits SDK)
- A/B testing
- Advanced analytics & reporting
- Geographic targeting
- Link partnerships
- Priority support

---

## Key Benefits

**For Bloggers & Publishers**
- Optimize ad placement for higher CTR
- Track affiliate performance
- Simple AdSense management

**For Community Owners**
- Monetize BuddyPress/bbPress natively
- Target by membership level
- Non-intrusive ad integration

**For Business Sites (Pro)**
- Build classified marketplaces
- Let advertisers manage themselves
- Passive income from listings

---

## Quick Stats

- **20+** Placement options
- **5** Ad types supported
- **0** Coding required
- **Unlimited** Ads in free version

---

## Pricing

| Plan | Price | Sites |
|------|-------|-------|
| Free | $0 | Unlimited |
| Pro Single | $[XX]/year | 1 |
| Pro Unlimited | $[XX]/year | Unlimited |

14-day money-back guarantee on all Pro purchases.

---

## Getting Started

1. Download from WordPress.org
2. Activate and run Setup Wizard
3. Create first ad in 60 seconds
4. Upgrade to Pro when needed

---

## Contact

**Website:** wbcomdesigns.com
**Support:** [support email]
**WordPress.org:** [plugin URL]

---

## Footer

Made by WBcom Designs | Trusted by [X]+ WordPress sites
